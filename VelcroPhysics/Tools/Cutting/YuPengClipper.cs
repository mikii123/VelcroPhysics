...
namespace VelcroPhysics.Tools.Cutting
{
    ...
    {
        private static readonly Fix64 ClipperEpsilonSquared = Fix64.FromRaw(512);
                                                   ...
                                                   out Vertices slicedPoly1, out Vertices slicedPoly2)
        {
            ...
            for (int i = 0; i < slicedPoly1.Count; ++i)
            {
                ...
                if ((slicedPoly1[iNext] - slicedPoly1[i]).MagnitudeSquared <= ClipperEpsilonSquared)
                {
            ...
            for (int i = 0; i < slicedPoly2.Count; ++i)
            {
                ...
                //If they are closer than the distance remove vertex
                if ((slicedPoly2[iNext] - slicedPoly2[i]).MagnitudeSquared <= ClipperEpsilonSquared)
                {
        ...
        private static Fix64 CalculateBeta(Vec2 point, Edge e, Fix64 coefficient)
        {
        ...
        private static Fix64 GetAlpha(Vec2 start, Vec2 end, Vec2 point)
        {
            return (point - start).MagnitudeSquared / (end - start).MagnitudeSquared;
        }
        ...
        private static Fix64 CalculateSimplexCoefficient(Vec2 a, Vec2 b, Vec2 c)
        {
        ...
        /// </returns>
        private static bool PointInSimplex(Vec2 point, Edge edge)
        {
        ...
        private static bool PointOnLineSegment(Vec2 start, Vec2 end, Vec2 point)
        {
            Vec2 segment = end - start;
            return MathUtils.Area(ref start, ref end, ref point) == Fix64.Zero &&
                   Vec2.Dot(point - start, segment) >= Fix64.Zero &&
                   Vec2.Dot(point - end, segment) <= Fix64.Zero;
        }

        private static bool VectorEqual(Vec2 vec1, Vec2 vec2)
        {
            return (vec2 - vec1).MagnitudeSquared <= ClipperEpsilonSquared;
        }
        ...
        private sealed class Edge
        {
            ...
            {
                return (EdgeStart + EdgeEnd) / Fix64.Two;
            }
