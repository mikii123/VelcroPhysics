...
namespace VelcroPhysics.Tools.Cutting.Simple
{
    public static class CuttingTools
    {
        ...
        public static void SplitShape(Fixture fixture, Vec2 entryPoint, Vec2 exitPoint, out Vertices first, out Vertices second)
        {
            ...
            foreach (Vec2 vertex in shape.Vertices)
            {
                if (vertex.Equals(localEntryPoint))
                    localEntryPoint -= new Vec2(Fix64.Zero, Fix64.Epsilon);

                if (vertex.Equals(localExitPoint))
                    localExitPoint += new Vec2(Fix64.Zero, Fix64.Epsilon);
            }
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                if (Vec2.Dot(MathUtils.Cross(localExitPoint - localEntryPoint, Fix64.One), vertices[i] - localEntryPoint) > Fix64.Epsilon)
                    n = 0;
                else
                    n = 1;
            ...
            for (int n = 0; n < 2; n++)
            {
                ...
                newPolygon[n][cutAdded[n]] += Fix64.Epsilon * offset;
                ...
                newPolygon[n][cutAdded[n] + 1] += Fix64.Epsilon * offset;
            }
        ...
        public static bool Cut(World world, Vec2 start, Vec2 end)
        {
            ...
            List<Vec2> entryPoints = new List<Vec2>();
            List<Vec2> exitPoints = new List<Vec2>();
