...
namespace VelcroPhysics.Tools.TextureTools
{
    ...
    {
        private static readonly int ClosepixelsLength = 8;
                                ...
                                bool? pixelOffsetOptimization, Matrix? transform)
        {
            ...
            if (hullTolerance.HasValue)
                ...
                HullTolerance = Fix64.One + Fix64.Half;
        ...
        /// <returns></returns>
        public static List<Vertices> DetectVertices(uint[] data, int width, Fix64 hullTolerance, byte alphaTolerance, bool multiPartDetection, bool holeDetection)
        {
            TextureConverter tc =
        ...
        public List<Vertices> DetectVertices()
        {
            ...
            List<Vec2> blackList = new List<Vec2>();
            ...
            do
            {
                ...
                else if (polygonEntrance.HasValue)
                {
                    ...
                    polygon = new Vertices(CreateSimplePolygon(polygonEntrance.Value, new Vec2(polygonEntrance.Value.X - Fix64.One, polygonEntrance.Value.Y)));
                }
                else
                    break;
                            ...
                            if (holeEntrance.HasValue)
                            {
                                if (!blackList.Contains(holeEntrance.Value))
                                {
                                    ...
                                    Vertices holePolygon = CreateSimplePolygon(holeEntrance.Value,
                                        new Vec2(holeEntrance.Value.X + Fix64.One, holeEntrance.Value.Y));
        ...
        private Vec2? SearchHoleEntrance(Vertices polygon, Vec2? lastHoleEntrance)
        {
            if (polygon == null)
        ...
        private bool DistanceToHullAcceptableHoles(Vertices polygon, Vec2 point, bool higherDetail)
        {
            if (polygon == null)
        ...
        private bool DistanceToHullAcceptable(Vertices polygon, Vec2 point, bool higherDetail)
        {
            if (polygon == null)
                throw new ArgumentNullException(nameof(polygon), "'polygon' can't be null.");
            ...
            if (higherDetail)
            {
                for (int i = 0; i < polygon.Count; i++)
                {
                    ...
                    if (LineUtils.DistanceBetweenPointAndLineSegment(ref point, ref edgeVertex1, ref edgeVertex2) <= _hullTolerance || Vec2.Distance(point, edgeVertex1) <= _hullTolerance)
                        return false;
        ...
        private bool InPolygon(Vertices polygon, Vec2 point)
        {
            bool inPolygon = !DistanceToHullAcceptableHoles(polygon, point, true);

            if (!inPolygon)
            {
                List<Fix64> xCoords = SearchCrossingEdgesHoles(polygon, (int)point.Y);
        ...
        private Vec2? GetTopMostVertex(Vertices vertices)
        {
            Fix64 topMostValue = Fix64.MaxValue;
            Vec2? topMost = null;
        ...
        {
            Fix64 returnValue = Fix64.MaxValue;
        ...
        {
            Fix64 returnValue = Fix64.MinValue;
        ...
        private List<Fix64> SearchCrossingEdgesHoles(Vertices polygon, int y)
        {
            ...
            List<Fix64> result = SearchCrossingEdges(polygon, y);
        ...
        private List<Fix64> SearchCrossingEdges(Vertices polygon, int y)
        {
            ...
            List<Fix64> edges = new List<Fix64>();
            ...
            if (polygon.Count > 2)
            {
                ...
                for (int i = 0; i < polygon.Count; i++)
                {
                    ...
                    if ((vertex1.Y >= (Fix64)y && vertex2.Y <= (Fix64)y) ||
                        (vertex1.Y <= (Fix64)y && vertex2.Y >= (Fix64)y))
                    {
                        ...
                        if (vertex1.Y != vertex2.Y)
                        {
                            ...
                            if (vertex1.Y == (Fix64)y)
                            {
                                ...
                                if (slope.Y > Fix64.Zero)
                                    addFind = (nextSlope.Y <= Fix64.Zero);
                                else
                                    addFind = (nextSlope.Y >= Fix64.Zero);
                            }

                            if (addFind)
                                edges.Add(((Fix64)y - vertex1.Y) / slope.Y * slope.X + vertex1.X); // Calculate and add the x coord.
                        }
                    }
        ...
        private bool SplitPolygonEdge(Vertices polygon, Vec2 coordInsideThePolygon, out int vertex1Index, out int vertex2Index)
        {
            Vec2 slope;
            ...
            Fix64 shortestDistance = Fix64.MaxValue;

            bool edgeCoordFound = false;
            Vec2 foundEdgeCoord = Vec2.Zero;

            List<Fix64> xCoords = SearchCrossingEdges(polygon, (int)coordInsideThePolygon.Y);
            ...
            if (xCoords != null && xCoords.Count > 1 && xCoords.Count % 2 == 0)
            {
                ...
                if (edgeCoordFound)
                {
                    ...
                    if (edgeFound)
                    {
                        ...
                        Vec2 tempVector = polygon[nearestEdgeVertex1Index];
        ...
        /// <returns></returns>
        private Vertices CreateSimplePolygon(Vec2 entrance, Vec2 last)
        {
            ...
            if (entrance == Vec2.Zero || !InBounds(ref entrance))
            {
                ...
                if (entranceFound)
                {
                    current = new Vec2(entrance.X - Fix64.One, entrance.Y);
                }
            }
            else
            {
        ...
        private bool SearchNearPixels(bool searchingForSolidPixel, ref Vec2 current, out Vec2 foundPixel)
        {
        ...
        private bool IsNearPixel(ref Vec2 current, ref Vec2 near)
        {
            for (int i = 0; i < ClosepixelsLength; i++)
            {
                int x = (int)current.X + _closePixels[i, 0];
        ...
        private bool SearchHullEntrance(out Vec2 entrance)
        {
        ...
        private bool SearchNextHullEntrance(List<Vertices> detectedPolygons, Vec2 start, out Vec2? entrance)
        {
            int x;
            ...
            for (int i = (int)start.X + (int)start.Y * _width; i <= _dataLength; i++)
            {
                if (IsSolid(ref i))
                {
                    ...
                    {
                        x = i % _width;
                        entrance = new Vec2((Fix64)x, (Fix64)(i - x) / (Fix64)_width);
        ...
        private bool GetNextHullPoint(ref Vec2 last, ref Vec2 current, out Vec2 next)
        {
            int x;
            int y;
        ...
        private bool SearchForOutstandingVertex(Vertices hullArea, out Vec2 outstanding)
        {
            ...
            if (hullArea.Count > 2)
            {
                ...
                for (int i = 1; i < hullAreaLastPoint; i++)
                {
                    ...
                    if (LineUtils.DistanceBetweenPointAndLineSegment(ref tempVector1, ref tempVec2, ref tempVector3) >= _hullTolerance)
                    {
        ...
        private int GetIndexOfFirstPixelToCheck(ref Vec2 last, ref Vec2 current)
        {
            // .: pixel
            // l: last position
            // c: current position
        ...
        public Fix64 HullTolerance
        {
            ...
            set
            {
                if (value > Fix64.Four)
                {
                    _hullTolerance = Fix64.Four;
                }
                else if (value <  (Fix64.Five + Fix64.Four) / new Fix64(10))
                {
                    _hullTolerance = (Fix64.Five + Fix64.Four) / new Fix64(10);
                }
                else
                {
        ...
        {
            return (coord.X >= Fix64.Zero && coord.X < (Fix64)_width && coord.Y >= Fix64.Zero && coord.Y < (Fix64)_height);
        }
