...
namespace VelcroPhysics.Collision.Broadphase
{
    ...
    {
        internal static readonly int NullNode = -1;
        ...
        {
            Vec2 p1 = input.Point1;
            Vec2 p2 = input.Point2;
            Vec2 r = p2 - p1;
            Debug.Assert(r.MagnitudeSquared > Fix64.Zero);
            r.Normalize();

            // v is perpendicular to the segment.
            Vec2 absV = MathUtils.Abs(new Vec2(-r.Y, r.X)); //Velcro: Inlined the 'v' variable
            ...
            while (_raycastStack.Count > 0)
            {
                ...
                Vec2 h = node.AABB.Extents;
                Fix64 separation = Fix64.Abs(Vec2.Dot(new Vec2(-r.Y, r.X), p1 - c)) - Vec2.Dot(absV, h);
                ...
                if (node.IsLeaf())
                {
                    ...
                    Fix64 value = callback(subInput, nodeId);
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            while (_nodes[index].IsLeaf() == false)
            {
                ...
                Fix64 cost = Fix64.Two * combinedArea;

                // Minimum cost of pushing the leaf further down the tree
                Fix64 inheritanceCost = Fix64.Two * (combinedArea - area);
