...
namespace VelcroPhysics.Collision.Narrowphase
{
    public static class CollidePolygon
    {
        ...
        public static void CollidePolygons(ref Manifold manifold, PolygonShape polyA, ref Transform xfA, PolygonShape polyB, ref Transform xfB)
        {
            ...
            int edgeA;
            Fix64 separationA = FindMaxSeparation(out edgeA, polyA, ref xfA, polyB, ref xfB);
            if (separationA > totalRadius)
                return;

            int edgeB;
            Fix64 separationB = FindMaxSeparation(out edgeB, polyB, ref xfB, polyA, ref xfA);
            ...
            bool flip;
            Fix64 k_tol = (Fix64.One / new Fix64(10)) * Settings.LinearSlop;
            ...
            Vec2 localNormal = MathUtils.Cross(localTangent, Fix64.One);
            Vec2 planePoint = Fix64.Half * (v11 + v12);

            Vec2 tangent = MathUtils.Mul(ref xf1.q, localTangent);
            Vec2 normal = MathUtils.Cross(tangent, Fix64.One);

            v11 = MathUtils.Mul(ref xf1, v11);
            v12 = MathUtils.Mul(ref xf1, v12);

            // Face offset.
            Fix64 frontOffset = Vec2.Dot(normal, v11);

            // Side offsets, extended by polytope skin thickness.
            Fix64 sideOffset1 = -Vec2.Dot(tangent, v11) + totalRadius;
            Fix64 sideOffset2 = Vec2.Dot(tangent, v12) + totalRadius;
            ...
            {
                Fix64 separation = Vec2.Dot(normal, clipPoints2[i].V) - frontOffset;
        ...
        private static Fix64 FindMaxSeparation(out int edgeIndex, PolygonShape poly1, ref Transform xf1, PolygonShape poly2, ref Transform xf2)
        {
            ...
            int bestIndex = 0;
            Fix64 maxSeparation = Fix64.MinValue;
            for (int i = 0; i < count1; ++i)
            {
                // Get poly1 normal in frame2.
                Vec2 n = MathUtils.Mul(ref xf.q, n1s[i]);
                Vec2 v1 = MathUtils.Mul(ref xf, v1s[i]);

                // Find deepest point for normal i.
                Fix64 si = Fix64.MaxValue;
                for (int j = 0; j < count2; ++j)
                {
                    Fix64 sij = Vec2.Dot(n, v2s[j] - v1);
                    if (sij < si)
                    {
                        si = sij;
                    }
        ...
        private static void FindIncidentEdge(out FixedArray2<ClipVertex> c, PolygonShape poly1, ref Transform xf1, int edge1, PolygonShape poly2, ref Transform xf2)
        {
            ...
            Vec2 normal1 = MathUtils.MulT(ref xf2.q, MathUtils.Mul(ref xf1.q, normals1[edge1]));

            // Find the incident edge on poly2.
            int index = 0;
            Fix64 minDot = Fix64.MaxValue;
