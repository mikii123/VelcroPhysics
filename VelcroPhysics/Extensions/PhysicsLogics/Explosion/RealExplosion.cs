...
namespace VelcroPhysics.Extensions.PhysicsLogics.Explosion
{
    ...
    public sealed class RealExplosion : PhysicsLogic
    {
        ...
        /// </summary>
        private static readonly Fix64 MaxEdgeOffset = Fix64.Pi / new Fix64(90);
        ...
        /// Defaults to 1/40.
        /// </summary>
        public Fix64 EdgeRatio = Fix64.One / new Fix64(40);
        ...
        /// </summary>
        public Fix64 MaxAngle = Fix64.Pi / new Fix64(15);
        ...
        public Dictionary<Fixture, Vec2> Activate(Vec2 pos, Fix64 radius, Fix64 maxForce)
        {
            AABB aabb;
            ...
            Dictionary<Fixture, Vec2> exploded = new Dictionary<Fixture, Vec2>(shapeCount + containedShapeCount);
            ...
            for (int i = 0; i < shapeCount; ++i)
            {
                ...
                if (cs != null)
                {
                    ...
                    Vec2 vec = Vec2.Zero + new Vec2(cs.Radius, Fix64.Zero);
                    v.Add(vec);
                    vec = Vec2.Zero + new Vec2(Fix64.Zero, cs.Radius);
                    v.Add(vec);
                ...
                {
                    Vec2 toCentroid = shapes[i].Body.GetWorldPoint(ps.MassData.Centroid) - pos;
                    ...
                    {
                        Vec2 toVertex = (shapes[i].Body.GetWorldPoint(ps.Vertices[j]) - pos);
                        Fix64 newAngle = Fix64.Atan2(toVertex.Y, toVertex.X);
                        Fix64 diff = (newAngle - angleToCentroid);

                        diff = (diff - Fix64.Pi) % (Fix64.PiTimes2);
            ...
            for (int i = 0; i < valIndex; ++i)
            {
                ...
                midpt = midpt / Fix64.Two;

                Vec2 p1 = pos;
                Vec2 p2 = radius * new Vec2((Fix64)Fix64.Cos(midpt), (Fix64)Fix64.Sin(midpt)) + pos;
            ...
            for (int i = 0; i < _data.Count; ++i)
            {
                ...
                Fix64 first = MathHelper.Min(MaxEdgeOffset, EdgeRatio * arclen);
                int insertedRays = (int)Fix64.Ceiling(((arclen - Fix64.Two * first) - (Fix64)(MinRays - 1) * MaxAngle) / MaxAngle);

                if (insertedRays < 0)
                    insertedRays = 0;

                Fix64 offset = (arclen - first * Fix64.Two) / ((Fix64)MinRays + (Fix64)insertedRays - Fix64.One);
                ...
                for (Fix64 j = _data[i].Min + first;
                     j < _data[i].Max || MathUtils.FloatEquals(j, _data[i].Max, Fix64.EpsilonSmaller);
                     j += offset)
                {
                    Vec2 p1 = pos;
                    Vec2 p2 = pos + radius * new Vec2((Fix64)Fix64.Cos(j), (Fix64)Fix64.Sin(j));
                    Vec2 hitpoint = Vec2.Zero;
                    ...
                    for (int x = 0; x < fl.Count; x++)
                    {
                        ...
                        Fix64 impulse = (arclen / (Fix64)(MinRays + insertedRays)) * maxForce * new Fix64(180) / Fix64.Pi * (Fix64.One - Fix64.Min(Fix64.One, minlambda));

                        // We Apply the impulse!!!
                        Vec2 vectImp = Vec2.Dot(impulse * new Vec2((Fix64)Fix64.Cos(j), (Fix64)Fix64.Sin(j)), -ro.Normal) * new Vec2((Fix64)Fix64.Cos(j), (Fix64)Fix64.Sin(j));
            ...
            for (int i = 0; i < containedShapeCount; ++i)
            {
                ...
                Fix64 impulse = (Fix64)MinRays * maxForce * new Fix64(180) / Fix64.Pi;
                Vec2 hitPoint;
                ...
                Vec2 vectImp = impulse * (hitPoint - pos);
