...
namespace VelcroPhysics.Shared
{
    ...
    public class Vertices : List<Vec2>
    {
        ...
        public Vec2 GetCentroid()
        {
            ...
            Vec2 c = Vec2.Zero;
            Fix64 area = Fix64.Zero;
            Fix64 inv3 = Fix64.One / Fix64.Three;
            ...
            for (int i = 0; i < Count; ++i)
            {
                ...
                Fix64 triangleArea = Fix64.Half * (current.X * next.Y - current.Y * next.X);
                area += triangleArea;
        ...
        {
            AABB aabb;
            Vec2 lowerBound = new Vec2(Fix64.MaxValue, Fix64.MaxValue);
            Vec2 upperBound = new Vec2(Fix64.MinValue, Fix64.MinValue);
        ...
        public void Rotate(Fix64 value)
        {
            ...
            Fix64 num1 = Fix64.Cos(value);
            Fix64 num2 = Fix64.Sin(value);

            for (int i = 0; i < Count; i++)
            {
                Vec2 position = this[i];
                this[i] = new Vec2((position.X * num1 + position.Y * -num2), (position.X * num2 + position.Y * num1));
            }
        ...
        public PolygonError CheckPolygon()
        {
            ...
            for (int i = 0; i < Count; ++i)
            {
                int next = i + 1 < Count ? i + 1 : 0;
                Vec2 edge = this[next] - this[i];
                if (edge.MagnitudeSquared <= Fix64.Epsilon * Fix64.Epsilon)
                {
        ...
        public void ProjectToAxis(ref Vec2 axis, out Fix64 min, out Fix64 max)
        {
            // To project a point on an axis use the dot product
            Fix64 dotProduct = Vec2.Dot(axis, this[0]);
            min = dotProduct;
            max = dotProduct;
        ...
        public int PointInPolygon(ref Vec2 point)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                if (area == Fix64.Zero && Vec2.Dot(point - p1, edge) >= Fix64.Zero && Vec2.Dot(point - p2, edge) <= Fix64.Zero)
                {
                    return 0;
                }
